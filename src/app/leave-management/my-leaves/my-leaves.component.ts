...
//   }
    ...
    /*deleteUser(userId: any, index: any, event: any): any {
        ...
        const dialogRef = this.dialog.open(PopupConfirmDeleteComponent, {
            width: "500px",
            data: {message: 'Are you sure you want to delete this Usere?'}
        });
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.apiService.deleteUser(userId).subscribe(data => {
                    let metaData: any = data.meta.message;
                    this.leaveDataSource.splice(index, 1);
                    this.leaveDataSource = new MatTableDataSource(this.leaveDataSource);
                    swal.fire(
                        'Deleted!',
                        metaData,
                        'success'
    ...
    cancelLeave(Id:any){
        const dialogRef = this.dialog.open(PopupCancelLeaveComponent, {
            width: "500px",
            data: {message: 'Are you sure you want to Cancel this leave?'}
        });
        dialogRef.afterClosed().subscribe(result => {
            if(result){
