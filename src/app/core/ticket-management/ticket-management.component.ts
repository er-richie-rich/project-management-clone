...
export class TicketManagementComponent implements OnInit, AfterViewInit, OnDestroy {
        ...
        this.apiService.getTicket(ticketId).subscribe(data => {
            ...
            if (ticketData.ticketStatus === 'Cancelled') {
                ...
                const dialogRef = this.dialog.open(CancelDeletePopupComponent, {
                    width: "500px",
                    data: {message: 'Are you sure you want to delete this ticket?',key:"Delete Ticket",icon:"delete-icon.png"}
                });
    ...
    cancelTicket(ticketId: any) {
        const dialogRef = this.dialog.open(CancelDeletePopupComponent, {
            width: "500px",
            data: {message: 'Are you sure! you want to cancel this ticket?', key:'Cancel Ticket',icon:"cancelImage.png"}
        });
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
